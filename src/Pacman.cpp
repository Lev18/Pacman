...
Pacman::Pacman(float pac_x, float pac_y, const std::string text_path) :
    ...
    if(!m_pac_texture.loadFromFile(curr_path / m_pac_path)){
            std::cerr << "Unable to load pacman from file\n";
            exit(1);
    }
...
void Pacman::animation_render(const int anim_speed) {
    if (m_frame_speed % anim_speed == 0) {
        m_pacman_ord += 16;
        if (m_pacman_ord >= 96) {
            m_pacman_ord = 0;
        }
    }
    m_pacman.setTextureRect(sf::IntRect(m_pacman_ord, m_pacman_dir, 16, 16));
}
